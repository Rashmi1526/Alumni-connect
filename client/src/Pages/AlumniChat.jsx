...
import { sendMessage, getPrivateConversation, getPrivateConversation2} from '../redux/action/alumniAction'
...
const Chat = (props) => {
    ...
    const formHandler = (e) => {
        ...
        if (message.trim().length > 0) {
            socket.emit("private message", {
                sender: store.alumni.alumni.alumni.name,
                message,
                room: room1
            })
            setMessage("")
            let messageObj = {
                roomId: room1,
                senderName: store.alumni.alumni.alumni.name,
                senderId: store.alumni.alumni.alumni._id,
                message,
                senderRegistrationNumber: store.alumni.alumni.alumni.registrationNumber,
    ...
    useEffect(() => {
        socket.on("new Message", (data) => {
            setOlderMessages(store.alumni.privateChat)
            setMessageArray([...messageArray, data])
        })
                ...
                <div className="container">
                    <div className="row">
                        ...
                        <div className="col-md-7">
                            {
                                store.alumni.privateChat.map((obj,index) =>
                                    <div key={index}>
