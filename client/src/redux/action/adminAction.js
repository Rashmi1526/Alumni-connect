...
//new
const adminAddAlumniFlag = (data) => {
    return {
        type: "ADMIN_ADD_ALUMNI_FLAG",
        payload: data
    }
}
...
//NEW
const adminGetAllAlumniHelper = (data) => {
    return {
        type: "GET_ALL_ALUMNI",
        payload: data
    }
...
//NEW
export const adminAddAlumni = (alumniCredential) => {
    return async (dispatch) => {
        try {
            const { data } = await axios({
                method: 'Post',
                url: url + "/api/admin/addAlumni",
                data: alumniCredential
            })
            dispatch(adminAddAlumniFlag(true))
...
//new
export const adminGetAllAlumni = (searchCredentials) => {
    return async (dispatch) => {
        try {
            const { data } = await axios({
                method: 'Post',
                url: url + "/api/admin/getAllAlumni",
                data: searchCredentials
            })
            dispatch(adminGetAllAlumniHelper(data.result))
        }
        catch (err) {
            dispatch({
                type: SET_ERRORS,
