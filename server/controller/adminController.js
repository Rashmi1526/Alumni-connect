...
const validateAlumniRegisterInput = require('../validation/alumniRegister')
...
module.exports = {
    ...
    addAlumni: async (req, res, next) => {
        try {
            const { errors, isValid } = validateAlumniRegisterInput(req.body);
            ...
            const alumnis = await Alumni.find({ department })
            let helper;
            if (alumnis.length < 10) {
                helper = "00" + alumnis.length.toString()
            }
            else if (alumnis.length < 100 && alumnis.length > 9) {
                helper = "0" + alumnis.length.toString()
            }
            else {
                helper = alumnis.length.toString()
            }
            let hashedPassword;
            hashedPassword = await bcrypt.hash(dob, 10)
            var date = new Date();
            ...
            res.status(200).json({ result: newAlumni })
        }
        catch (err) {
            res.status(400).json({ message: `error in adding alumni", ${err.message}` })
        }
    ...
    getAllAlumni: async (req, res, next) => {
        try {
            const { department, year } = req.body
            const allAlumnis = await Alumni.find({ department, year })
            res.status(200).json({ result: allAlumnis })
        }
        catch (err) {
            console.log("Error in gettting all alumnis", err.message)
        }
    },
